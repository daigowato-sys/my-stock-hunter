...
# --- 2. 強化版：無料ニュース感情分析エンジン ---
def analyze_sentiment_free(news_list):
    if not news_list or not isinstance(news_list, list):
        return "【判定：中立 😐】\n\n現在、有効なニュースは見当たりません。"
    
    pos_words = ["増益", "上方修正", "最高益", "提携", "買収", "拡大", "好調", "反発", "期待", "上昇", "黒字", "配当増", "自社株買い", "buy", "positive", "growth", "surge", "gain"]
    neg_words = ["減益", "下方修正", "赤字", "不祥事", "懸念", "失速", "続落", "売り", "下落", "マイナス", "低迷", "急落", "sell", "negative", "loss", "risk", "decline"]
...
with tab1:
    ...
    if st.button('全銘柄スキャン開始！'):
        if not target_stocks:
            ...
            with st.spinner(f'{len(target_stocks)} 銘柄を解析中...'):
                all_data = []
                ...
                if not results.empty:
                    ...
                    for s in sectors:
                        sector_df = results[results['業種'] == s]
                        count = len(sector_df)
                        with st.expander(f"📁 {s} ({count}銘柄)"):
                            display_df = sector_df.drop(columns=["概要", "ニュース", "業種"])
                            st.dataframe(display_df.sort_values(by=sort_col, ascending=(mode=="底値狙い（逆張り）")).style.background_gradient(subset=['騰落率(%)', '配当(%)'], cmap='RdYlGn'))
            ...
            try:
                ...
                if len(df) < 50: st.error("データ不足です。")
                    ...
                    if results:
                        ...
                        c3.metric("3日後勝率", f"{len([r for r in results if r > 0])/len(results)*100:.1f}%", f"{sum(results)/len(results):.2f}% (平均利益)")
                    fig = go.Figure()
